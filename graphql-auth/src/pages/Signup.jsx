...
const handleSignup = async (e) => { e.preventDefault(); try { 
    const res = await fetch("https://api-qa.seamasterai.com/graphql",
         { method: "POST",
             headers: { "Content-Type": "application/json" }, 
             body: JSON.stringify({ query: SIGNUP_MUTATION,
                 variables:
                  { input: 
                    { username, email, password }, }, }), }); const result = await res.json(); if (result?.data?.register?.jwt) { alert("Signup successful. Please login."); navigate("/"); } else { alert("Signup failed: " + result?.errors?.[0]?.message); } } catch (err) { alert("Signup failed"); } };
